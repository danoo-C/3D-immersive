...
class Panel(QFrame):
    """A titled region of the window: the header every panel carries.

    Split out of `Placeholder` when the media pool became the first real
    panel, so the header's style has one home rather than a copy per panel.
    `body()` is where a panel puts its content.
    """
    ...
    def __init__(self, title: str, parent: QWidget | None = None) -> None:
        ...
        # box: it is where the real panel's title lives, so the shell reads as
        # unfinished rather than as badly laid out.
    ...
    def retheme(self) -> None:
        ...
        Inline stylesheets, set at construction, which is why this method has
        to exist: D-76 made the *accessors* read at call time and nothing made
        a widget re-read what it had already baked in. The theme picker walks
        the tree calling this, and a widget that does not have it stays
        visibly the old colour - which is the point of making the protocol a
        capability rather than a subscription.
        """
        self.setStyleSheet(
            f"#Panel {{"
