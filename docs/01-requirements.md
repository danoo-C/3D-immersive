...
| D-79 | The bundled theme has **no fallback palette**. An unreadable or malformed bundled `.3dimtheme` refuses to start, with a message naming the resource | M9 phase 3's Scope asks for "the fallback if the bundled resource is somehow unreadable", and read as a fallback *theme* that cannot coexist with the same phase's first acceptance line: a palette to fall back to is thirteen colours in `theme.py`, and the acceptance is that `theme.py` contains no hex at all. So the fallback is a behaviour rather than a palette. It is a **packaging** failure and not a runtime condition — the file is inside the wheel, and if it is missing then everything else the application is about to do is broken too. A fallback palette would also be a second definition of the default, kept in a second place and used by nobody in any run that works, so the first time the two disagreed the disagreement would be invisible. And `theme.py` already behaves exactly this way: `_template()` reads `app.qss` through `importlib.resources` with no fallback, and a missing stylesheet takes the application down today — a bundled theme that fell back where the bundled stylesheet does not would be two rules for one category of thing. What prevents it is not a fallback but `tests/test_package.py` and CI installing the wheel rather than the source tree, which is D-27's stated benefit and is already collected. |
| D-80 | The built-in theme is a **cached function** in `theme_io`, `builtin()`, and `theme.py` reaches it through a deferred import | Reading the default from a file needs somewhere to put the reader, and both obvious homes make a cycle: `theme_io` already imports `theme`, so a reader in `theme.py` would need `SCHEMA_VERSION` and the document mapping back from `theme_io`, while a reader in `theme_io` leaves `theme.active()` needing a default it cannot import at module level. The third option — move the format's constants into `theme.py` — looks cleanest and is worst, because it splits the `.3dimtheme` format across two modules, which is what D-77 put it in one place to avoid. A function-level absolute import is still greppable, which is what D-28 cares about, and one commented line is a smaller cost than a format split in half. It is a function rather than a module constant for three reasons: `theme.py` already uses exactly that pattern in `_template()` for exactly this kind of thing; a module-level read does file I/O at import and can **raise at import**, turning a packaging fault into an ImportError inside a traceback rather than a startup message that names the file; and reading when asked rather than capturing at import is D-76's instinct one layer out. The cache is what keeps it one read per process, and like `icons.icon()` it has to be cleared by any test that swaps the resource underneath it. |
